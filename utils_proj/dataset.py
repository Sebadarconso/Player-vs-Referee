...
def print_sample(sample, classes, flag = True):
    ...
    img = draw_bounding_boxes( img_int, sample[1]['boxes'], [classes[i-1] for i in sample[1]['labels']], colors = color, width=2,
        font="calibri.ttf", font_size=20).permute(1, 2, 0)
    if flag:
        plt.imshow(img)
        plt.show()
    else:
...
class Dataset(datasets.VisionDataset):
        def __init__(self, root, type = "train", transform=None, target_transform=None, transforms=None):
            # the 3 transform parameters are required for datasets.VisionDataset
            super().__init__(root, transforms, transform, target_transform)
            self.type = type #train, valid, test
            self.root = root
            self.coco = COCO(os.path.join(self.root, self.type, "annotations.json")) # annotations stored here
